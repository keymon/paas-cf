...
CERT_ID=$(aws s3 cp "s3://mmg-${DEPLOY_ENV}-state/cf-certs.tfstate"  - \
  | jq -r ".modules[].outputs.system_domain_cert_id.value")

if [ -z "${CERT_ID// }" ] || [ "${CERT_ID}" == "null" ]; then
  echo "Failed to obtain system_domain_cert_id from s3://mmg-${DEPLOY_ENV}-state/cf-certs.tfstate"
  exit 1
fi
...
terraform remote config \
  -backend=s3 \
  -backend-config="bucket=mmg-${DEPLOY_ENV}-state" \
