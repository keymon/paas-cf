...
aws s3 cp "s3://mmg-${DEPLOY_ENV}-state/bosh_id_rsa" ${BOSH_KEY} && chmod 400 ${BOSH_KEY}
aws s3 cp "s3://mmg-${DEPLOY_ENV}-state/concourse_id_rsa" ${CONCOURSE_KEY} && chmod 400 ${CONCOURSE_KEY}
...
echo
aws s3 cp "s3://mmg-${DEPLOY_ENV}-state/bosh-secrets.yml" - | \
